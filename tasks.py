...
@task
def download_weights(ctx: Context, kind: str = "base", repo_id: str = "", out: str = "models") -> None:
    """
    Download released model weights from the Hugging Face Hub.

    Example:
        uv run invoke download-weights                         # base checkpoint
        uv run invoke download-weights --kind translation      # translation head
        uv run invoke download-weights --repo-id user/repo     # custom repo
    """
    parts = [f"--kind {kind}", f"--out {out}"]
    if repo_id:
        parts.append(f"--repo-id {repo_id}")
    ctx.run(
        "uv run python -m fett.cli.download_weights " + " ".join(parts),
        echo=True, pty=not WINDOWS,
    )


@task
def publish_weights(ctx: Context, ckpt: str, kind: str = "base", repo_id: str = "", private: bool = False) -> None:
    """
    Upload a checkpoint to the Hugging Face Hub. Requires HF_TOKEN env var.
    ...
    """
    parts = [f"--ckpt {ckpt}", f"--kind {kind}"]
    if repo_id:
        parts.append(f"--repo-id {repo_id}")
    if private:
        parts.append("--private")
    ctx.run(
        "uv run python -m fett.cli.publish_weights " + " ".join(parts),
        echo=True, pty=not WINDOWS,
    )
